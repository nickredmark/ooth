...
async function massageInsertUserFields(fields: any) {
  let data: { userMeta: { create: any[] } } = { userMeta: { create: [] } };
    ...
    if (prismaUserFields.indexOf(key) == -1) {
      ...
      for (var i = keyParts.length - 1; i >= 0; i--) {
        if (i == keyParts.length - 1) {
          ...
          temp = { key: keyParts[i], child: { create: [ temp ] } };
        }
      }
      data.userMeta.create.push(temp); 
    } else {
      (<any>data)[key] = fields[key];
    } 
  }
...
function userMetaToObject(userMeta: any) {
  ...
  for (var i = 0, len = userMeta.length; i < len; i++) {
    ...
    if( userMeta[i].child.length > 0 ) {
      ...
      (<any>f)[key] = userMetaToObject(userMeta[i].child);
    } else {
      (<any>f)[key] = userMeta[i].value;
    }
  }
  return f;
}
...
export class OothPrisma {
  ...
  public insertUser = async (fields: { [key: string]: StrategyValues }) => {
    const data = await massageInsertUserFields(fields);
    try {
